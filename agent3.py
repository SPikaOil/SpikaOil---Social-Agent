...
def haal_product_op(product_id=None, product_naam=None, project_id=None):
    ...
    # Als project_id meegegeven — gebruik project research ipv product research
    if project_id:
        projecten = supabase_get("projecten", f"select=*&id=eq.{project_id}")
        if projecten:
            proj = projecten[0]
            print(f"[Agent 3] Project: {proj.get('project_naam')} ({proj.get('doelland')})")
            # Overschrijf research data met project data
            product = product.copy()
            product["hook_bibliotheek"]    = proj.get("hook_bibliotheek")    or product.get("hook_bibliotheek")
            product["cta_bibliotheek"]     = proj.get("cta_bibliotheek")     or product.get("cta_bibliotheek")
            product["video_format_regels"] = proj.get("video_format_regels") or product.get("video_format_regels")
            product["research_rapport"]    = proj.get("research_rapport")    or product.get("research_rapport")
            product["doelland"]            = proj.get("doelland",  product.get("doelland", ""))
            product["doeltaal"]            = proj.get("doeltaal",  product.get("doeltaal", ""))
            product["soul_id"]             = proj.get("soul_id",   product.get("soul_id", ""))
            product["brand_lock_template"] = proj.get("brand_lock_template") or product.get("brand_lock_template")
            product["_project_id"]         = project_id
...
def sla_scripts_op(product_id, product_naam, scripts, project_id=None):
...
def run(product_id=None, product_naam=None, project_id=None):
    """Voert Agent 3 uit voor een specifiek product of project."""
    print("\n" + "=" * 60)
    print("  AGENT 3 — SCRIPT MAKER")
    print("=" * 60)

    # Haal product op
    product = haal_product_op(product_id, product_naam, project_id)
    if not product:
    ...
    # Sla op in Supabase
    run_id = sla_scripts_op(pid, naam, scripts, project_id=project_id)
