...
def run(product_id=None, product_naam=None, project_id=None):
    ...
    Als project_id meegegeven wordt, slaat hij op in projecten tabel.
    Anders in producten tabel.
    """
    print("\n" + "=" * 60)
    ...
    # Bepaal of we voor project of product werken
    tabel  = "projecten" if project_id else "producten"
    filter = f"id=eq.{project_id}" if project_id else f"id=eq.{pid}"
    context = f"project {project_id}" if project_id else f"product {pid}"

    print(f"[Agent 1] Soul ID aanmaken voor {context}")

    # Check of Soul ID al bestaat
    if project_id:
        projecten = supabase_get("projecten", f"select=soul_id&id=eq.{project_id}")
        bestaande_soul_id = projecten[0].get("soul_id") if projecten else ""
    else:
        bestaande_soul_id = product.get("soul_id", "")

    if bestaande_soul_id:
        print(f"[Agent 1] Soul ID al aangemaakt: {bestaande_soul_id} — overslaan")
        return {"success": True, "bericht": "Soul ID al beschikbaar", "soul_id": bestaande_soul_id}
    ...
    # Pas persona aan voor project (ander land/taal)
    if project_id:
        projecten = supabase_get("projecten", f"select=doelland,doeltaal,project_naam&id=eq.{project_id}")
        if projecten:
            proj = projecten[0]
            soul_persona = soul_persona.copy()
            soul_persona["doelland"] = proj.get("doelland", "")
            soul_persona["project_naam"] = proj.get("project_naam", "")
            print(f"[Agent 1] Project: {proj.get('project_naam')} ({proj.get('doelland')})")

    print(f"[Agent 1] Persona: {soul_persona.get('naam', 'Onbekend')}")

    # Genereer brand lock template
    brand_lock = genereer_brand_lock_template(product)
    supabase_patch(tabel, filter, {"brand_lock_template": brand_lock})
    ...
    soul_naam = f"{naam}_{project_id}_Soul" if project_id else f"{naam}_Soul"
    soul_id   = registreer_soul_id(soul_naam, portret_urls)
    ...
    supabase_patch(tabel, filter, update_data)

    print("\n" + "=" * 60)
    print("  AGENT 1 KLAAR")
    print(f"  {context}: {naam}")
    print(f"  Soul ID: {soul_id or 'Brand lock opgeslagen'}")
    print("=" * 60)
    ...
    return {
        ...
        "soul_id":    soul_id,
        "brand_lock": brand_lock,
        "bericht":    f"Soul ID aangemaakt voor {context}"
    }
