...
def run(run_id=None, product_id=None, project_id=None):
    """Voert Agent 4 uit voor een product of project."""
    print("\n" + "=" * 60)
    ...
    # Als project meegegeven — gebruik project soul_id en brand_lock
    if project_id:
        projecten = supabase_get("projecten", f"select=soul_id,brand_lock_template&id=eq.{project_id}")
        if projecten:
            proj = projecten[0]
            if proj.get("soul_id"):
                product = product.copy()
                product["soul_id"] = proj.get("soul_id")
                print(f"[Agent 4] Project Soul ID: {proj.get('soul_id')}")
            if proj.get("brand_lock_template"):
                product["brand_lock_template"] = proj.get("brand_lock_template")
